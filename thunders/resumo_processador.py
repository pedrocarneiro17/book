    ...
    try:
        ...
        for row in ws.iter_rows(values_only=True):
            if row[0] == "TOTAL GERAL":
                ...
                total_book  = pd.to_numeric(row[11], errors='coerce')   # col 12 → Valor NF
                return (
        ...
        print(f"[Thunders Resumo] Aba '{sheet_name}' não encontrada.")
        return 0, 0


def _apply_styles(ws):
    """Aplica formatação na aba de resumo Thunders."""
...
def criar_aba_resumo_thunders(workbook, data_compras, data_vendas):
    """Cria a aba de Resumo para o modelo Thunders (somente Livro x Book)."""
    print("\n--- INICIANDO CRIAÇÃO DA ABA DE RESUMO THUNDERS ---")
    ...
    ws['B2'] = "Conciliação Livro Fiscal x Book Thunders"
    ...
    _apply_styles(ws)
    print("[Thunders Resumo] Aba de Resumo criada com sucesso.")
    return workbook
